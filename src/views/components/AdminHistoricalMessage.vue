<template>
  <div class="box-right">
    <div class="box-button" style="text-align:center;">
      ...
      <el-input style="width: 15rem;" v-model="messageAll" clearable placeholder="请输入消息内容" class="input"
    ...
    <div class="box-operation" v-show="addDisplay">
      <div class="box-operation-info">
        <h3>添加信息</h3>
        <div><el-input v-model="message" clearable placeholder="请输入信息" class="input"></el-input></div>
        <div><el-input v-model="time" clearable placeholder="请输入时间" class="input" type="datetime-local"
            step="1"></el-input></div>
    ...
    <el-table border style="width: 100%" class="right" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="message" label="消息内容" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <div v-if="scope.row.message.includes('/img/')">
            <img v-image-preview :src="scope.row.message" style="max-width: 100px; max-height: 100px;" />
          </div>
          <div v-else>
            {{ scope.row.message }}
          </div>
    ...
    <el-table border style="width: 100%" class="right" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="time" label="发送时间" width="220">
        <template slot-scope="scope">
          <el-input type="datetime-local" step="1" :value="formattedTime(scope.row.time)"
            @input="val => scope.row.time = new Date(val).toISOString()" />
        </template>
      </el-table-column>
      <el-table-column prop="username" label="用户名" width="180">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
      ...
      if (this.multipleSelect.length === 0) {
        ...
        this.$confirm("此操作将永久删除, 是否继续?", "警告", {
          ...
          for (var i = 0; i < this.multipleSelect.length; i++) {
            axios.get('http://192.168.1.105:3001/message/del', {
              params: {
    ...
    all() {
      axios.get('http://192.168.1.105:3001/message/all').then(res => {
    ...
    get() {    //查询操作
      this.currentPage = 1;
      axios.get('http://192.168.1.105:3001/message/get', {
        params: {
    ...
    add() {   //添加操作
      axios.get('http://192.168.1.105:3001/message/add', {
        params: {
