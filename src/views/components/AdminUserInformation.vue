<template>
  <div class="box-right">
    <div class="box-button" style="text-align:center;">
      ...
      <el-input style="width: 9rem;" v-model="usernameAll" clearable placeholder="请输入用户名" class="input" v-enter="get" />
      <el-radio-group v-model="onlineAll">
      ...
      <el-button type="primary" @click="addDisplay = !addDisplay" icon="el-icon-plus">添加</el-button>
      <el-button type="danger" @click="multipleDel" icon="el-icon-warning">批量删除</el-button>
    </div>

    <!-- 添加窗口 -->
    <div class="box-operation" v-show="addDisplay">
      <div class="box-operation-info">
        <h3>添加信息</h3>
        <div><el-input v-model="username" clearable placeholder="请输入用户名" class="input"></el-input></div>
        <div><el-input v-model="password" clearable placeholder="请输入密码" class="input"></el-input></div>
        <div><el-input v-model="reg_time" clearable placeholder="请输入注册时间" class="input" type="datetime-local"
            step="1"></el-input></div>
        <div class="box-operation-button">
          <el-button type="primary" @click="add">添加</el-button>
          <el-button type="danger" @click="addDisplay = !addDisplay">取消</el-button>
        </div>
      </div>
    </div>
    ...
    <el-table border style="width: 100%;" class="right" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="avatarurl" label="用户头像" width="200">
        <template slot-scope="scope">
          <img v-image-preview :src="scope.row.avatarurl" min-width="100" height="100" />
        </template>
      </el-table-column>
      <el-table-column prop="reg_time" label="注册时间" :formatter="formatTime" width="220" />
      <el-table-column prop="online" label="在线状态" width="180">
        <template slot-scope="scope">
          <el-radio-group v-model="scope.row.online" class="prohibit">
            <el-radio-button label="0">离线</el-radio-button>
            <el-radio-button label="1">在线</el-radio-button>
          </el-radio-group>
        </template>
      ...
      <el-table-column prop="operation" label="操作" width="300">
        <template slot-scope="scope">
          <el-button @click="updateOperate">修改</el-button>
    ...
    <el-table border style="width: 100%" class="right" @selection-change="handleSelectionChange"
      ...
      <el-table-column prop="reg_time" label="注册时间" width="220">
        <template slot-scope="scope">
          <el-input type="datetime-local" step="1" :value="formattedTime(scope.row.reg_time)"
            @input="val => scope.row.reg_time = new Date(val).toISOString()" />
        </template>
      </el-table-column>
      <el-table-column prop="online" label="在线状态" width="180">
        <template slot-scope="scope">
          <el-radio-group v-model="scope.row.online">
            <el-radio-button label="0">离线</el-radio-button>
            <el-radio-button label="1">在线</el-radio-button>
          </el-radio-group>
        </template>
      ...
      <el-table-column prop="operation" label="操作" width="300">
        <template slot-scope="scope">
          <el-button type="success" @click="buttonDisplaySaved(scope.row)">保存</el-button>
...
export default {
  ...
  methods: {
      ...
      if (this.multipleSelect.length === 0) {
        ...
        this.$confirm("此操作将永久删除, 是否继续?", "警告", {
          ...
          for (var i = 0; i < this.multipleSelect.length; i++) {
            axios.get('http://192.168.1.105:3001/usersinfo/del', {
              params: {
    ...
    all() {
      axios.get('http://192.168.1.105:3001/usersinfo/all').then(res => {
    ...
    get() {    //查询操作
      this.currentPage = 1;
      axios.get('http://192.168.1.105:3001/usersinfo/get', {
        params: {
          username: this.usernameAll,
          online: this.onlineAll
        },
      }).then(res => {
        this.info = res.data;
      }).catch(err => {
    ...
    add() {   //添加操作
      axios.get('http://192.168.1.105:3001/usersinfo/add', {
        params: {
    ...
    //修改操作
    buttonDisplaySaved(row) {
      const originalDate = new Date(row.reg_time);
      originalDate.setHours(originalDate.getHours() + 8);
      const formattedDate = originalDate.toISOString().slice(0, 19).replace('T', ' ');
      axios.get('http://192.168.1.105:3001/usersinfo/update', {
        params: {
          username: row.username,
          online: row.online,
          reg_time: formattedDate,
          id: row.id,
        }
      }).then(res => {
        // console.log(res.data);
